...
class ServerlessAwsAutoResourceNamesPlugin {
        ...
        if(this.isAwsTemplate) {
            this.hooks = {
                ...
                "after:aws:package:finalize:mergeCustomProviderResources": () => {
                    ...
                    this.applyOnOutputs(this.service["provider"]["compiledCloudFormationTemplate"]["Output"]);
                    this.applyOnOutputs(this.service["provider"]["coreCloudFormationTemplate"]["Output"]);
                },
            };
    ...
    applyOnJsonFile(path){
        ...
        this.applyOnResources(template["Resources"]);
        this.applyOnOutputs(template["Output"]);

        fs.writeFileSync(path, JSON.stringify(template, null, 2));
    }

    applyOnOutputs(outputs){
        Object.keys(outputs).forEach((key) => {
            const output = outputs[key];
            output.Export = output.Export || {};
            output.Name = output.Name || this.config.prefix + kebabCase(key);
        });
    }
