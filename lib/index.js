...
class TypeSpec {
     ...
     */
    applyType(element, logicalElement, prefix, logicalName){
        if(this.isNameInserted()){
            element[this.getNameProp()] = (logicalElement || {})[this.getNameProp()] || this.getNameValue(prefix, logicalName);
        }
    }
...
class ServerlessAwsAutoResourceNamesPlugin {
    ...
    applyOnResource(resources, templateResources, logicalName){
        ...
        typeSpecs.forEach((spec) => {
            if(spec.isApplicable(new TypeID(typeRoot, typeProvider, typeName))){
                ...
                spec.applyType(resource["Properties"], templateResource["Properties"], this.config.prefix, logicalName);
    ...
    applyOnOutput(outputs, logicalName){
        ...
        if(output["Export"]) {
            ...
            spec.applyType(output["Export"], output["Export"], this.config.exportPrefix || this.config.prefix, logicalName);
        }
    }
    ...
    initializeConfig(){
        ...
        const { prefix = "", exportPrefix = undefined, generateExports = false } = (custom && custom["awsAutoResourceNames"]) || {};
        ...
        // check export generation property
        if(typeof generateExports !== "boolean"){
            this.throwError("generate exports must be boolean");
        }

        // set the generated config
        this.config = {
            prefix, exportPrefix, generateExports
        };
    }
}
