...
class ServerlessAwsAutoResourceNamesPlugin {
    ...
    applyOnOutput(outputs, logicalName){
        ...
        if(output["Export"]) {
            ...
            spec.applyType(output["Export"], output["Export"], this.config.outputPrefix || this.config.prefix, logicalName);
        }
    }
    ...
    initializeConfig(){
        ...
        const { prefix = "", outputPrefix = undefined } = (custom && custom["awsAutoResourceNames"]) || {};
